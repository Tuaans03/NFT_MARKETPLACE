...
const createSynchronize = async( synchronizeBody) =>{
    Synchronize.create(synchronizeBody);
}
...
const getLastSynchronizeById = async(id) =>{
    return Synchronize.findOne().sort({createAt:-1});
};
...
const deleteSynchronizeById = async(synchronizeId) =>{
    const synchronize = await getLastSynchronizeById(synchronizeId);
    if(!synchronize){
        throw new ApiError(httpStatus.NOT_FOUND,'Synchronize not found');
    }
