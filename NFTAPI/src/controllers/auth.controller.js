...
const register = catchAsync( async(req,res) =>{
    ...
    let _user = user;
    _user.id = user._id;
    const tokens = await tokenService.generateAuthToken(_user);
...
const forgotPassword = catchAsync( async(req,res) =>{
    ...
    res.status(httpStatus.OK).json(response(httpStatus.OK),'Vui lòng kiểm tra hộp thư của bạn để xác nhận việc đổi mật khẩu');
})
...
const sendVerificationEmail = catchAsync( async(req,res) =>{
    ...
    res.status(httpStatus.OK).json( response(httpStatus.OK,'Vui lòng kiểm tra hộp thư để xác minh tài khoản'));
})
